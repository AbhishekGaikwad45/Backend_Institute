...
//reassign faculty
    ...
    @PostMapping("/reassign-faculty")
    public ResponseEntity<?> reassignFaculty(@RequestBody Map<String, String> req) {

        String batchCode = req.get("batchCode");
        String newFacultyCode = req.get("facultyCode");

        Batch batch = batchService.getBatchByCode(batchCode);
        if (batch == null)
            return ResponseEntity.status(404).body(Map.of("message", "Batch not found"));

        Faculty faculty = facultyService.getFacultyByCode(newFacultyCode);
        if (faculty == null)
            return ResponseEntity.status(404).body(Map.of("message", "Faculty not found"));

        if (batch.getFacultyCode() == null)
            return ResponseEntity.ok(Map.of("message", "No faculty assigned yet"));

        if (batch.getFacultyCode().equals(newFacultyCode))
            return ResponseEntity.ok(Map.of("message", "Same faculty already assigned"));

        // ✅ CORRECT CALL
        batchService.reassignFacultyByCode(batchCode, newFacultyCode);

        return ResponseEntity.ok(
                Map.of("message", "Faculty reassigned successfully")
        );
    }
    ...
    public ResponseEntity<?> removeStudent(
        ...
        batchService.removeStudentFromBatch(studentId, batchCode);

        return ResponseEntity.ok(
                Map.of("message", "Student removed from batch")
        );
    }
