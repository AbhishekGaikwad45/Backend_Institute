...
//reassign batch and update batch by faculty code
    public Batch reassignFacultyByCode(String batchCode, String facultyCode) {

        Batch batch = repo.findByBatchCode(batchCode);
        if (batch == null) return null;

        batch.setFacultyCode(facultyCode); // 🔥 overwrite
        return repo.save(batch);           // 🔥 DB update
    }

//    remove student from batch
    public List<Student> getStudentsByBatchCode(String batchCode) {

        List<StudentBatch> mappings =
                studentBatchRepo.findByBatchCode(batchCode);

        List<Student> students = new ArrayList<>();

        for (StudentBatch sb : mappings) {
            Student student =
                    studentRepo.findByStudentIdIgnoreCase(sb.getStudentId());
            if (student != null) {
    ...
    @Transactional
    public void removeStudentFromBatch(String studentId, String batchCode) {
        studentBatchRepo.deleteByStudentIdAndBatchCode(studentId, batchCode);
    }
